...
import {
  ...
  ReceivedLoginData,
  Login,
} from "@interfaces/UserLoginInterface";

const bcrypt = require("bcrypt");

export default async function Handler(
  req: NextApiRequest,
  res: NextApiResponse<LoggingInterface | ReposneInterface | any>
) {
  await mongoose.connect(process.env.DATABASE_URL);
  const { email, password }: Login = JSON.parse(req.body);
  if ((await CheckIfEmailExists(email)) !== false) {
    const UserData: ReceivedLoginData = (await CheckIfEmailExists(
      email
    )) as ReceivedLoginData;
    ...
    bcrypt.compare(
      PasswordGivenByUser,
      PasswordInDatabase,
      async function (err: any, result: boolean) {
        if (err) {
          console.log(err);
        ...
        if (result) {
          ...
          res
            .status(200)
            .json({
              message: { text: "niepoprawne email bądź hasło", status: 0 },
            });
        }
      }
    );
    ...
    res
      .status(200)
      .json({
        message: { text: "nie ma uzytkownika o podanym mailu", status: 0 },
      });
  }
}
